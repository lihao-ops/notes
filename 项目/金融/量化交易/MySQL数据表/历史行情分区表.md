热数据表（2024年1月 - 2025年12月，未压缩，快速查询）---温数据表（2020年1月 - 2023年12月，压缩存储，中速查询）----通过XXL-job定时转档每月1号将热数据表中的超期范围的数据转移到温数据表。温数据表中超过的3年范围内的数据删除。-因为行情数据超过两年以上回测的价值就会大打折扣。到了五年以上基本上数据就不会有什么意义了。或者我再抽离一下将近三个月的数据临再建一个hot数据表？这个表里将核心数据字段加起来在原有的基础上再建立覆盖索引怎么样？
...
## 温数据表(2020年1月~2023年12月，压缩存储，中速查询)
...
```java
小李，你这个判断是**绝对正确的**。
只要涉及“大表 + 仍然有实时写入”，**单纯 SQL INSERT…SELECT 就是灾难**——慢、锁、回滚代价高、业务受影响。

你现在的方向：

# ✅ 使用在线迁移工具（gh-ost / pt-online-schema-change）进行“表平移 + 分区化 + 不停机”

这是专业 DBA、金融数据系统常用方式。

你说你想：

> “假定目前这个表还有实时插入的数据，用工具迁移，Windows + WSL2 + gh-ost”
...
tb_quotation_history_trend_202002
...
​```
...
​```
tb_quotation_history_warm   （分区表 + 压缩）
tb_quotation_history_hot     （分区表）
​```
...
* **结构完全不同（老表：无分区、新表：分区+压缩）**
* **需要先创建新表，然后把数据实时同步**
* **旧表仍然持续写入行情数据（插入率很高）**

gh-ost 的核心：

### ✔ 利用 MySQL binlog 增量同步

迁移期间：

* 全表初次 copy：快
* binlog 捕获增量写入：确保数据一致性
* 最后切换：瞬间完成
...
如果你的 MySQL 写入变高，会自动暂停迁移，避免业务抖动。

### ✔ 可随时回滚、暂停、继续

### ✔ 有“影子表 shadow table”安全防护

所以我说：
**这就是大厂 DBA 做迁移的主流方案，没有之一。**

---

# 🏗️ **第二件事：你的迁移逻辑（来源表 → 目标分区表）**
...
​```
tb_quotation_history_trend_202001 → tb_quotation_history_warm
tb_quotation_history_trend_202002 → tb_quotation_history_warm
...
​```

因为温表分区按 trade_date 自动落盘，无需你管理。

### 🔴 实时数据不在这些表内（热表另处理）

---

# 🖥️ **第三件事：环境准备（Windows + WSL2）**
...
## ✔ Step 3：在 WSL2 内安装 gh-ost

​```bash
sudo apt update
sudo apt install gh-ost
​```

## ✔ Step 4：在 WSL2 中连接 Windows MySQL（本机 3306）
...
# 🏎️ **第四件事：开始迁移（以 202001 表为例）**

## ✔ 1) 先创建 shadow table（gh-ost 自动做）
...
--max-lag-millis=1500 \
--verbose \
--initially-drop-old-table=no \
--initially-drop-ghost-table=yes \
--serve-socket-file=/tmp/gh-ost.202001.sock \
--serve-socket-file-verbose \
--alter="ENGINE=InnoDB ROW_FORMAT=COMPRESSED KEY_BLOCK_SIZE=8"
​```
...
* 创建 `_ghost` 影子表（结构为温表结构）
* 从原表 full copy 数据
* 持续同步 binlog 增量
* 数据一致后，自动 “swap table”

**不会锁大表，不会中断写入，不会影响行情落盘。**
...
### 🔧 Java 调用 gh-ost 的整套迁移工具（更适合生产）

### 🔧 XXL-job 每个月自动归档脚本

### 🔧 迁移流程图（Mermaid 8.8.3 中文）
