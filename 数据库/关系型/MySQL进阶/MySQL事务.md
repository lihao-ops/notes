...
查询的范围数据，在事务的执行前后执行后出现了其它事务对该范围的数据新增或者删除。

主要是通过间隙锁来解决，在查询范围内容的数据遵循前开后闭的原则，也就是说查询SQL

```sql
select * from `table` where id > 500 and id < 1000
```

且当前id=1000是存在的，否则会为该条上记录锁，从而形成临键锁，默认会为该查询范围新增间隙锁。防止该范围内的数据行出现新增或者删除。从而防止出现幻读的现象。

>为什么MVCC 不能解决幻读?

MVCC多版本并发控制，从在当前事务修改前和修改后，通过undo.log对应的版本链控制查看的是同一个版本，但是这样的话相当于在该范围内的更新不能及时反馈到该事务版本，导致覆盖更新等现象，只是解决了并发隔离和稳定性，但是并没有解决幻读问题。
