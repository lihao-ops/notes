...
##### 1️⃣ InnoDB 加锁到底发生在什么时机？优化器会影响加锁行为吗？

##### 2️⃣ 行锁、间隙锁、临键锁分别是什么？在什么语句下会出现？

##### 3️⃣ 为什么同一条 SQL 在 RC 下不会产生间隙锁，而在 RR 下会？

##### 4️⃣ 用主键等值查询一定只加行锁吗？给我一个能反例的场景。

##### 5️⃣ 非唯一索引 + 范围查询一定会加 next-key lock 吗？什么时候不会？

------

#### 🧨 Part 2：死锁机制（从原理到复现）

##### 6️⃣ 说出死锁的 4 个必要条件，解释哪一个条件最容易被打破？

##### 7️⃣ 不同顺序加锁为什么会 100% 造成死锁？给出一个你亲手复现过的例子。

##### 8️⃣ InnoDB 使用什么算法检测死锁？为什么说是“图算法”？

##### 9️⃣ InnoDB 如何决定回滚哪一个事务？请列出影响决策的因素（不少于 3 个）。

##### 🔟 为什么两个主键等值 FOR UPDATE 也能出现死锁？

（提示：资源访问顺序）

###### 1️⃣1️⃣ 死锁日志中的 “heap no” 是什么？它对分析死锁有什么意义？

###### 1️⃣2️⃣ 有没有可能出现 3 个事务以上的循环等待？如何构造？

------

#### 🧨 Part 3：锁等待 & 长事务（生产事故高频点）

##### 1️⃣3️⃣ 为什么“长事务”会导致数据库性能暴跌？请从 MVCC 角度解释。

##### 1️⃣4️⃣ 如何利用 `information_schema.innodb_trx` 快速定位长事务？

##### 1️⃣5️⃣ 为什么一个普通的 SELECT 也可能被阻塞？给出两个能复现的场景。

##### 1️⃣6️⃣ data_lock_waits + data_locks 能定位哪个表、哪行正在被锁？

请说出必要字段及其意义（不需要答案，只描述题意）。

##### 1️⃣7️⃣ 解释：为什么间隙锁不会互斥，却仍然可能触发死锁？

##### 1️⃣8️⃣ 锁等待（lock_wait_timeout）和死锁（deadlock）有什么本质区别？

------

#### 🧨 **Part 4：乐观锁 vs 悲观锁（深入业务）**

##### 1️⃣9️⃣ 乐观锁真的“无锁”吗？它到底有没有锁？是在什么时候加锁的？

##### 2️⃣0️⃣ 为什么乐观锁不适合多行原子扣减？给出实际业务逻辑案例说明。

##### 2️⃣1️⃣ 如果多个线程高并发 CAS 失败，吞吐量会发生什么变化？原因是什么？

##### 2️⃣2️⃣ 设计一个“商品库存扣减”接口，用悲观锁如何避免死锁？

##### 2️⃣3️⃣ 在什么场景下，悲观锁一定优于乐观锁？

------

#### 🧨 **Part 5：批量操作与事务控制（性能优化）**

##### 2️⃣4️⃣ 为什么批量写性能比单条写快一个数量级？请从 redo/binlog/事务 提到三方面。

##### 2️⃣5️⃣ 批量写是否更容易死锁？为什么？

##### 2️⃣6️⃣ 在分批写入中，批次（batch_size）过大/过小分别会带来什么问题？

------

#### 🧨 Part 6：连接池调优（HikariCP 面试真题）

##### 2️⃣7️⃣ 如何压测确定 HikariCP 的最佳 maximum-pool-size？给出你的压测流程。

##### 2️⃣8️⃣ 为什么 max-lifetime 不能大于 MySQL 的 wait_timeout？

##### 2️⃣9️⃣ 如果连接池耗尽，Spring Boot 服务会出现什么现象？如何定位？

##### 3️⃣0️⃣ 如何识别“连接泄漏”？HikariCP 提供了哪些检测工具？

------

#### 🧨 Part 7：高级 InnoDB 行为（真正拉开候选人差距）

##### 3️⃣1️⃣ 为什么 DELETE + 范围条件会锁全索引？能展示一个你复现的例子吗？

##### 3️⃣2️⃣ 在 RR 下，为什么 `ORDER BY ... LIMIT` 会触发 Next-Key Lock？

##### 3️⃣3️⃣ 插入语句（INSERT）为什么也会加间隙锁？在哪些场景发生？

##### 3️⃣4️⃣ explain format=json 中 optimizer 的选择过程如何影响加锁策略？

##### 3️⃣5️⃣ 为什么 change buffer（自适应哈希索引）可能改变加锁表现？

------

#### 🧨 **Part 8：系统设计（大厂二面/三面常问）**

##### 3️⃣6️⃣ 设计一个“高并发库存系统”，如何做到：

- 防超卖
- 无死锁
- 高吞吐
- 可扩展
   请描述至少 3 套方案（悲观锁/乐观锁/Redis/消息队列/预扣减）。

##### 3️⃣7️⃣ 如何在超高并发秒杀场景避免“锁表”？请给出对应 SQL。

##### 3️⃣8️⃣ 如果你的订单系统长期出现锁等待，你会如何排查？说出完整体系化步骤。

##### 3️⃣9️⃣ 如何通过索引设计减少锁冲突？列举至少 3 条有效策略。

##### 4️⃣0️⃣ 如何通过架构设计（缓存 + MQ + DB）减少锁竞争？

------

#### 🧨 **Part 9：生产事故题（重点）**

##### 4️⃣1️⃣ 某天库存表 QPS 暴跌 80%，CPU 升高，你怀疑是锁问题，你如何排查？

（越细越好）

##### 4️⃣2️⃣ 某业务出现 ghost record（幻读）导致扣减异常，你如何复现和解决？

##### 4️⃣3️⃣ 为什么线上会出现“间隙锁导致吞吐量暴跌”？给出分析路径。

##### 4️⃣4️⃣ DELETE + PK 范围条件导致全表锁住，你怎么解释？

##### 4️⃣5️⃣ 为什么线上会突然出现大量锁等待，但没有死锁日志？你如何分析？

------

#### 🧨 **Part 10：终极问题（阿里/字节 3 面深问）**

##### 4️⃣6️⃣ 如果让你实现一个简化版的“行级锁管理器”，你会怎么设计？

##### 4️⃣7️⃣ MySQL 为什么要使用 Next-Key Lock？不用它会有什么灾难？

##### 4️⃣8️⃣ 给你一条 SQL，你能否在执行前预测它可能产生什么锁？

##### 4️⃣9️⃣ 如果让你实现一套无锁库存系统（Lock-Free），你会如何设计版本机制？

##### 5️⃣0️⃣ MySQL 为什么要保留 undo log 版本链？它对锁冲突有什么影响？
