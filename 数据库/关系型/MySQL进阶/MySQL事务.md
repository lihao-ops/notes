...
一致性，MySQL事务的四大特性之一，是由多个因素共同保障的从一种状态到另一种状态时，是相对守恒的。

也就是说：

- 事务A初始值=1000，事务B初始值=2000，
- 当事务A执行-700，然后事务B执行+700，
- 此时事务A变成另一种状态300，事务B变成了2700。
- 守恒在于事务A+事务B总值仍旧为3000。



> 一致性主要是由以下几种因素共同努力的结果

1. 约束：主键约束，或者自定义的其它约束：例如当前字段不可为0等
2. 原子性：保证的事务不会处于执行一半的情况，通过`undo.log`实现要么全部执行，要么回滚。
3. 隔离性：通过MVCC维持多版本并发控制，在高并发的环境下，保证事务与事务执行期间会有各自的版本，不会造成相互干扰。
4. 持久型：主要是通过执行前写日志，`redo.log`来实现只要事务前写入日志，执行成功，哪怕在突然断电后重启也会恢复，具体的条件，是看当前执行的事务是否写入到了`redo.log`。



>一致性与隔离性的区别在于主要是解决不同维度的问题

- 一致性：主要是解决相对守恒的问题
- 隔离性：主要解决的是并发环境下事务与事务执行相关之间不受干扰的问题
